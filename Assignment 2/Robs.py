...
def averageCE(target, prediction):
    ...
    return -np.sum(target*np.log(s))/target.shape[0]
...
def calcGradient(x0,s1,x1,s2,x2,y,b1,w1,b2,w2):
    #s2_exp = np.exp(s2)
    # 1xd2 or 1xC
    # ds2 = np.multiply(gradCE(y,s2),np.mean((s2_exp/np.sum(s2_exp)),axis=0,keepdims=True))
    print(gradSM(s2).shape)
    ds2 = np.matmul(gradCE(y,s2),gradSM(s2))
...
#    avgX1 = np.transpose(np.mean(x1,axis=0,keepdims=True))
    # d1xd2
    #print(avgX1.shape, ds2.shape)
    dw2 = np.matmul(x1,ds2)
...
x2 = softmax(s2)
print("Final Training CE:", averageCE(newtrain,s2))
...
s2 = computeLayer(x1,tw2,tb2)
x2 = softmax(s2)
print("Final Validation CE:", averageCE(newvalid,s2))
...
s2 = computeLayer(x1,tw2,tb2)
x2 = softmax(s2)
print("Final Testing CE:", averageCE(newtest,s2))
