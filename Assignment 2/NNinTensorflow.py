...
#    X = tf.reshape(X, shape=[-1, 28, 28, 1])
    ...
    W1 = tf.get_variable(name = "W1",dtype = tf.float32, shape=[3,3,1,32], initializer=tf.contrib.layers.xavier_initializer_conv2d(),trainable=True)
    b1 = tf.get_variable(name = "b1",dtype = tf.float32, shape=[32], initializer=tf.contrib.layers.xavier_initializer(),trainable=True)
    conv = tf.nn.conv2d(X, W1, strides=[1, 1, 1, 1], padding='SAME')
    conv = tf.nn.bias_add(conv,b1)
    conv_relu = tf.nn.relu(conv)
    mean, var = tf.nn.moments(conv_relu,axes = [0,1,2,3])
...
#    x1 = tf.layers.max_pooling2d(x1,pool_size = [2,2], strides = 2, padding = 'SAME')
    ...
    W2 = tf.get_variable(name = "W2",dtype = tf.float32, shape=[6272,784], initializer=tf.contrib.layers.xavier_initializer(uniform=False),trainable=True)
    b2 = tf.get_variable(name = "b2", dtype = tf.float32,shape=[784], initializer=tf.constant_initializer(1),trainable=True)
    
    dropout_rate = 0.5 # == 1 - keep_prob
    x1_dropped = tf.nn.dropout(x1_flat, keep_prob = 1 - dropout_rate)
...
#    s2 = tf.matmul(x1_flat,W2)  
    ...
    x2 = tf.nn.relu(s2)
    W3 = tf.get_variable(name = "W3", dtype = tf.float32, shape=[784,10], initializer=tf.contrib.layers.xavier_initializer(uniform=False), trainable=True)
    b3 = tf.get_variable(name = "b3",dtype = tf.float32, shape=[10], initializer=tf.constant_initializer(1), trainable=True)
