...
def MoG():
    ...
    MU = tf.get_variable('mean',dtype = tf.float32,shape = [K,D], initializer = tf.initializers.random_normal(stddev = 2))
...
validloss,valid_belong = sess.run([error,belongs], feed_dict = {X:val_data})
